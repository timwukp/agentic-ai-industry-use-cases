...
def _apply_fill(portfolio_id: str, symbol: str, side: str, quantity: int, price: float):
    ...
    if side == "BUY":
        if item:
            ...
            tbl.put_item(
                Item=to_decimal(
                    {**key, "symbol": symbol, "quantity": quantity, "avgCost": price}
                )
            )
        return None
    if not item:
...
def place_order(
    ...
    if order_type not in VALID_TYPES:
        return tool_error(
            f"Invalid order_type: {order_type}", valid=sorted(VALID_TYPES)
        )
    ...
    table("ORDERS_TABLE").put_item(
        Item=to_decimal({k: v for k, v in order.items() if v is not None})
    )
    return tool_ok(
        {
            "order_id": order_id,
...
def cancel_order(order_id: str) -> dict:
    ...
    if item["status"] != "OPEN":
        return tool_error(
            f"Order {order_id} is {item['status']}; only OPEN orders can be cancelled"
        )
    tbl.update_item(
    ...
    return tool_ok(
        {"order_id": order_id, "status": "CANCELLED", "cancelled_at": _now()}
    )
    ...
    return tool_ok(
        {
            ...
            "summary": {
                ...
                "total_commissions": round(
                    sum(o.get("commission", 0) for o in orders), 2
                ),
            },
        }
    )
