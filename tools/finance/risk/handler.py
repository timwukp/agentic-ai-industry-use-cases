...
def calculate_var(portfolio_value: float, confidence_level: float = 0.95,
                  time_horizon_days: int = 1, annual_volatility: float = 0.18) -> dict:
...
def stress_test_portfolio(portfolio_value: float, scenario: str = "all") -> dict:
    ...
    if scenario == "all":
        ...
        return tool_error(f"Unknown scenario: {scenario}",
    ...
    return tool_ok({
        "portfolio_value": portfolio_value,
        "scenarios": results,
        "worst_case": {"scenario": worst_name, "loss": results[worst_name]["estimated_loss"]},
        "recommendation": ("Consider hedging strategies (puts, inverse ETFs) if stress losses "
...
def analyze_portfolio_risk(positions: str) -> dict:
    ...
    avg_corr = 0.6
    w_vols = [(a["weight_pct"] / 100) * (a["annual_volatility_pct"] / 100) for a in analysis]
    ...
    return tool_ok({
        ...
        "concentration_risk": {
            "top_position_weight_pct": round(max(a["weight_pct"] for a in analysis), 2),
            "herfindahl_index": round(sum((a["weight_pct"] / 100) ** 2 for a in analysis), 4),
        },
        "positions": analysis,
    }, simulated=True)


def monte_carlo_simulation(portfolio_value: float, annual_return: float = 0.08,
                           annual_volatility: float = 0.15, years: int = 10,
    ...
    rng = random.Random(f"mc|{portfolio_value}|{annual_return}|{annual_volatility}|{years}|{n}")
    # GBM sampled annually (sufficient for percentile outcomes, 250x faster than daily)
    finals = []
    for _ in range(n):
        value = portfolio_value
        for _ in range(years):
            value *= math.exp(rng.gauss(annual_return - annual_volatility ** 2 / 2,
                                        annual_volatility))
        finals.append(value)
    finals.sort()
    percentiles = {f"p{p}": round(finals[int(n * p / 100)], 2)
                   for p in (5, 10, 25, 50, 75, 90, 95)}
    prob_loss = sum(1 for v in finals if v < portfolio_value) / n
    prob_double = sum(1 for v in finals if v >= portfolio_value * 2) / n
    return tool_ok({
        "parameters": {
            "initial_value": portfolio_value, "annual_return": annual_return,
            "annual_volatility": annual_volatility, "years": years, "num_simulations": n,
        },
        "results": {
