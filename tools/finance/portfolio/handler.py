...
def calculate_pnl(portfolio_id: str = "default", period: str = "month") -> dict:
    ...
    value_then = round(
        sum(p["quantity"] * sim.close_price(p["symbol"], then) for p in positions), 2
    )
    ...
    return tool_ok(
        {
            ...
            "performance": {
                "period_return_pct": (
                    round((value_now / value_then - 1) * 100, 2) if value_then else 0
                ),
    ...
    return tool_ok(
        {
            ...
            "concentration": {
                "top_position_weight_pct": round(
                    max(p["market_value"] for p in enriched) / total_value * 100, 2
                ),
...
def suggest_rebalancing(
    portfolio_id: str = "default", target_allocation: str = "{}"
) -> dict:
    ...
    return tool_ok(
        {
            ...
            "estimated_total_turnover": round(
                sum(t["estimated_trade_value"] for t in trades), 2
            ),
