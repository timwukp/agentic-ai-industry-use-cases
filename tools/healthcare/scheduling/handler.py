...
def schedule_appointment(
    ...
    r = _rng(
        "schedule", patient_id, provider_id.upper(), appointment_type, preferred_date
    )
...
def get_provider_availability(provider_id: str, date_range: str = "7") -> dict:
    provider = PROVIDER_DIRECTORY.get(
        provider_id.upper(),
        {
    ...
    while current <= end_date:
        if current.weekday() < 5:
            ...
            slots = sorted(
                r.sample(morning, k=r.randint(1, 5))
            ...
            "summary": (
                {
                    ...
                    "total_available_slots": sum(
                        d["total_open_slots"] for d in availability
                    ),
                    "earliest_available": availability[0]["available_slots"][0]["time"],
                }
                if availability
                else {"message": "No available slots in the requested date range."}
            ),
        ...
        appointments.append(
            {
                ...
                "location": (
                    "Telehealth (virtual)"
                    if "Telehealth" in appt_type
                    else r.choice(LOCATIONS)
                ),
                "duration_minutes": r.choice([15, 20, 30, 45, 60]),
                "status": r.choice(
                    ["Confirmed", "Confirmed", "Confirmed", "Pending Confirmation"]
                ),
    ...
    return tool_ok(
        {
            ...
            "reminders": {
                ...
                "message": (
                    "Some appointments have not received reminders yet."
                    if pending
...
def send_appointment_reminder(appointment_id: str) -> dict:
    ...
    time_str = (
        f"{r.choice([8, 9, 10, 11, 13, 14, 15]):02d}:{r.choice([0, 15, 30, 45]):02d}"
    )
    ...
    return tool_ok(
        {
            ...
            "delivery_channel": r.choice(
                ["SMS", "Email", "Patient Portal", "SMS and Email"]
            ),
            "appointment_details": {
                "date": appt_date.strftime("%Y-%m-%d"),
                "time": time_str,
