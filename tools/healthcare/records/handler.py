...
MEDICATIONS = [
    {
        ...
        "route": "oral",
        "indication": "Type 2 Diabetes (E11.9)",
    ...
    },
    {
        ...
        "route": "oral",
        "indication": "Hyperlipidemia (E78.5)",
        "prescriber": "Dr. Sarah Chen, MD",
    ...
    },
    {
        ...
        "dosage": "20mg",
        "frequency": "QD (before breakfast)",
        "route": "oral",
        "indication": "GERD (K21.0)",
    ...
    },
    {
        ...
        "route": "inhaled",
        "indication": "Asthma (J45.20)",
        "prescriber": "Dr. Priya Patel, MD",
    ...
    },
    {
        ...
        "class": "SSRI",
        "dosage": "100mg",
        "frequency": "QD (once daily, morning)",
        "route": "oral",
        "indication": "Generalized Anxiety (F41.1)",
        "prescriber": "Dr. Lisa Kim, MD",
    ...
    },
    {
        ...
        "frequency": "QD (once daily)",
        "route": "oral",
        "indication": "Vitamin D deficiency (E55.9)",
        "prescriber": "Dr. Sarah Chen, MD",
...
def get_patient_summary(patient_id: str) -> dict:
    ...
    for _ in range(r.randint(2, 5)):
        recent_visits.append(
            {
                "date": (today - timedelta(days=r.randint(7, 180))).strftime(
                    "%Y-%m-%d"
                ),
                "type": r.choice(
                    [
                ...
                "provider": r.choice(
                    ["Dr. Chen", "Dr. Patel", "Dr. Wilson", "NP Rodriguez"]
                ),
    ...
    return tool_ok(
        {
            ...
            "demographics": {
                ...
                "age": age,
                "sex": sex,
                "date_of_birth": (today - timedelta(days=age * 365)).strftime(
                    "%Y-%m-%d"
                ),
                "blood_type": r.choice(BLOOD_TYPES),
                "primary_language": r.choice(
                    ["English", "English", "Spanish", "Mandarin"]
                ),
                "insurance": r.choice(
                    [
                        "Blue Cross PPO",
            ...
            "vitals_last_recorded": {
                ...
                "height": r.choice(
                    ["5ft 2in", "5ft 5in", "5ft 8in", "5ft 10in", "6ft 0in", "6ft 2in"]
                ),
        ...
        results.append(
            {
                ...
                "date": (today - timedelta(days=r.randint(1, 730))).strftime(
                    "%Y-%m-%d"
                ),
...
def get_medication_list(patient_id: str) -> dict:
    ...
    for med in meds:
        ...
        med["last_filled"] = (today - timedelta(days=r.randint(5, 60))).strftime(
            "%Y-%m-%d"
        )
        med["pharmacy"] = r.choice(
            [
                "CVS Pharmacy #4521",
                "Walgreens #1089",
                "Rite Aid #723",
    ...
    return tool_ok(
        {
            ...
            "last_reconciliation": (today - timedelta(days=r.randint(1, 90))).strftime(
                "%Y-%m-%d"
            ),
...
def get_lab_results(patient_id: str, days: int = 90) -> dict:
        ...
        results.append(
            {
                ...
                "result_date": (collection + timedelta(days=r.randint(1, 3))).strftime(
                    "%Y-%m-%d"
                ),
                "ordering_provider": r.choice(
                    [
                        "Dr. Sarah Chen, MD",
                        "Dr. Priya Patel, MD",
                        "Dr. James Wilson, MD",
    ...
    abnormal = sum(
        1 for p in results for t in p["tests"] if t["flag"] in ("HIGH", "LOW")
    )
    critical = [
        t["test"]
        for p in results
        for t in p["tests"]
        if t["flag"] == "HIGH"
        and t["test"] in ("Glucose", "Potassium", "Creatinine", "WBC")
    ]
    ...
    return tool_ok(
        {
            ...
            "recommendation": (
                "Critical values detected - notify ordering provider immediately."
                if critical
                else "Results reviewed - no critical values."
            ),
