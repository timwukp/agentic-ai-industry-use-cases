...
DISCLAIMER = (
    "Predictive model for clinical decision support. "
...
def get_patient_analytics(patient_id: str) -> dict:
    ...
    months = [
        (today - timedelta(days=i * 30)).strftime("%Y-%m") for i in range(12, 0, -1)
    ]
        ...
        bp_trend.append(
            {
                "month": month,
                "systolic": max(
                    105, base_systolic + r.randint(-8, 8) + int(drift * i * 2)
                ),
                "diastolic": max(
                    60, base_diastolic + r.randint(-5, 5) + int(drift * i)
                ),
            }
        )
        bmi_trend.append(
            {
                "month": month,
                "value": round(
                    max(18.5, base_bmi + r.uniform(-0.3, 0.3) + drift * i * 0.2), 1
                ),
            }
        )
        if i % 3 == 0:
            a1c_trend.append(
                {
                    "month": month,
                    "value": round(
                        max(4.5, base_a1c + r.uniform(-0.3, 0.3) + drift * i * 0.1), 1
                    ),
                }
            )
        weight_trend.append(
            {
                "month": month,
                "value": max(
                    100, base_weight + r.randint(-3, 3) + int(drift * i * 1.5)
                ),
            }
        )
        if i % 6 == 0:
            ldl_trend.append(
                {
                    "month": month,
                    "value": max(
                        40, base_ldl + r.randint(-15, 15) + int(drift * i * 2)
                    ),
                }
            )
    ...
    return tool_ok(
        {
            ...
            "trends": {
                "blood_pressure": {
                    ...
                    "trend_direction": (
                        "improving"
                        if bp_trend[-1]["systolic"] < bp_trend[0]["systolic"]
                        else "worsening"
                    ),
                },
                "bmi": {
                    ...
                    "trend_direction": (
                        "improving"
                        if bmi_trend[-1]["value"] < bmi_trend[0]["value"]
                        else (
                            "stable"
                            if abs(bmi_trend[-1]["value"] - bmi_trend[0]["value"]) < 0.5
                            else "worsening"
                        )
                    ),
                },
                "hemoglobin_a1c": {
                    ...
                    "trend_direction": (
                        "improving"
                        if len(a1c_trend) >= 2
                        and a1c_trend[-1]["value"] < a1c_trend[0]["value"]
                        else "stable"
                    ),
                },
                "weight": {
                    ...
                    "trend_direction": (
                        "decreasing" if weight_change < 0 else "increasing"
                    ),
                },
                "ldl_cholesterol": {
                    "data": ldl_trend,
                    ...
                    "status": (
                        "AT TARGET" if ldl_trend[-1]["value"] < 100 else "ABOVE TARGET"
                    ),
                },
            },
            "clinical_insights": [
    ...
    return tool_ok(
        {
            "practice_panel": {
                ...
                "sex_distribution": {
                    "male": f"{male_pct}%",
                    "female": f"{100 - male_pct}%",
                },
            },
            "chronic_disease_prevalence": {
                "diabetes_type_2": {
                    "count": r.randint(300, 800),
                    "rate": f"{round(r.uniform(12, 22), 1)}%",
                },
                "hypertension": {
                    "count": r.randint(500, 1200),
                    "rate": f"{round(r.uniform(25, 40), 1)}%",
                },
                "hyperlipidemia": {
                    "count": r.randint(400, 1000),
                    "rate": f"{round(r.uniform(20, 35), 1)}%",
                },
                "obesity_bmi_30_plus": {
                    "count": r.randint(600, 1500),
                    "rate": f"{round(r.uniform(30, 42), 1)}%",
                },
                "depression_anxiety": {
                    "count": r.randint(200, 600),
                    "rate": f"{round(r.uniform(10, 20), 1)}%",
                },
                "asthma_copd": {
                    "count": r.randint(150, 500),
                    "rate": f"{round(r.uniform(8, 16), 1)}%",
                },
                "heart_failure": {
                    "count": r.randint(50, 200),
                    "rate": f"{round(r.uniform(3, 8), 1)}%",
                },
            },
            "quality_measures_hedis": {
                "diabetes_a1c_control_lt8": {
                    "performance": f"{r.randint(65, 88)}%",
                    "target": "75%",
                    "national_avg": "72%",
                },
                "blood_pressure_control": {
                    "performance": f"{r.randint(60, 85)}%",
                    "target": "70%",
                    "national_avg": "68%",
                },
                "breast_cancer_screening": {
                    "performance": f"{r.randint(55, 82)}%",
                    "target": "75%",
                    "national_avg": "73%",
                },
                "colorectal_cancer_screening": {
                    "performance": f"{r.randint(50, 78)}%",
                    "target": "70%",
                    "national_avg": "65%",
                },
                "flu_vaccination_rate": {
                    "performance": f"{r.randint(40, 72)}%",
                    "target": "70%",
                    "national_avg": "52%",
                },
                "depression_screening": {
                    "performance": f"{r.randint(55, 85)}%",
                    "target": "80%",
                    "national_avg": "65%",
                },
            },
            ...
            "improvement_opportunities": [
                {
                    ...
                    "gap": "A1C testing overdue for 12% of diabetic patients",
                    "impact": "HIGH",
                },
                {
                    "area": "Preventive Screening",
                    "gap": "Colorectal cancer screening below national target",
                    "impact": "HIGH",
                },
                {
                    "area": "Chronic Care",
                    "gap": "15% of hypertensive patients without BP reading in 6 months",
                    "impact": "MEDIUM",
                },
                {
                    "area": "Immunizations",
                    "gap": "Flu vaccination rate below organizational target",
                    "impact": "MEDIUM",
                },
                {
                    "area": "Behavioral Health",
                    "gap": "Depression screening gap in 20% of eligible patients",
                    "impact": "MEDIUM",
                },
            ],
            "report_period": (
...
def get_readmission_risk(patient_id: str) -> dict:
    ...
    contributing = [
        ...
        for f in [
            ...
            (
                f"{prior_admissions} hospital admissions in past 12 months"
                if prior_admissions
                else None
            ),
            f"{ed_visits} ED visits in past 6 months" if ed_visits > 1 else None,
            (
                "Lives alone - limited caregiver support"
                if social["lives_alone"]
                else None
            ),
            (
                "Transportation barriers to follow-up"
                if social["transportation_barriers"]
                else None
            ),
            (
                "Limited health literacy"
                if social["health_literacy"] == "limited"
                else None
            ),
        ]
        if f
    ...
    if risk_level in ("MODERATE", "HIGH"):
        interventions += [
            {
                "intervention": "Transitional care management (TCM) visit within 7 days",
                "priority": "HIGH",
            },
            {
                "intervention": "Medication reconciliation by clinical pharmacist",
                "priority": "HIGH",
            },
            {
                "intervention": "Follow-up phone call within 48 hours of discharge",
                "priority": "HIGH",
            },
        ]
    if risk_level == "HIGH":
        interventions += [
            {
                "intervention": "Home health referral for post-discharge monitoring",
                "priority": "HIGH",
            },
            {"intervention": "Care coordinator assignment", "priority": "HIGH"},
            {
                "intervention": "Enroll in remote patient monitoring program",
                "priority": "MEDIUM",
            },
        ]
    if social["lives_alone"]:
        interventions.append(
            {
                "intervention": "Social work consult for community support services",
                "priority": "MEDIUM",
            }
        )
    if social["transportation_barriers"]:
        interventions.append(
            {
                "intervention": "Arrange medical transportation for follow-up visits",
                "priority": "MEDIUM",
            }
        )
    interventions.append(
        {
            "intervention": "Patient education on warning signs requiring ED return",
            "priority": "MEDIUM",
        }
    )
    ...
    all_gaps = [
        {
            ...
            "last_completed": past(380, 800),
            "due_date": past(10, 200),
            "status": "OVERDUE",
            "priority": "HIGH",
        ...
        },
        {
            ...
            "last_completed": past(200, 400),
            "due_date": past(10, 100),
            "status": "OVERDUE",
            "priority": "HIGH",
        ...
        },
        {
            ...
            "last_completed": past(365, 500),
            "due_date": "Next flu season (Oct)",
            "status": "DUE SOON",
            "priority": "MEDIUM",
        ...
        },
        {
            ...
            "last_completed": past(3650, 5000),
            "due_date": past(10, 365),
            "status": "OVERDUE",
            "priority": "HIGH",
    ...
    return tool_ok(
        {
            ...
            "care_gaps": selected,
            "quality_impact": {
                "hedis_measures_affected": sum(
                    1 for g in selected if "HEDIS" in g["quality_measure"]
                ),
