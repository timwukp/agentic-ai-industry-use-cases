...
def get_demand_trends(category: str, period: str = "month") -> dict:
    ...
    return tool_ok({
        ...
        "top_growing_skus": [
            {"sku": f"SKU-{r.randint(100, 999)}", "growth_pct": round(r.uniform(10, 50), 1)}
            for _ in range(5)
        ],
        "declining_skus": [
            {"sku": f"SKU-{r.randint(100, 999)}", "decline_pct": round(r.uniform(-30, -5), 1)}
            for _ in range(3)
        ],
...
def auto_reorder(sku: str) -> dict:
    ...
    return tool_ok({
        ...
        "order_details": {
            ...
            "supplier_name": r.choice(["GlobalSupply Co", "Pacific Distributors",
                                       "Premier Wholesale", "Atlas Trading"]),
        },
        "calculations": {
            "eoq": eoq,
        ...
        "cost_analysis": {
            ...
            "total_annual_inventory_cost": round(
                annual_demand / eoq * ordering_cost + eoq / 2 * unit_cost * holding_cost_pct, 2),
        },
        "expected_delivery": (today + timedelta(days=lead_time_days)).strftime("%Y-%m-%d"),
    }, simulated=True)
