...
def transfer_stock(sku: str, from_location: str, to_location: str, quantity: int) -> dict:
    ...
    today = _today()
    r = _rng("transfer_stock", sku, from_location, to_location, quantity,
    ...
    return tool_ok({
        ...
        "quantity": quantity,
        "estimated_delivery": (today + timedelta(days=r.randint(1, 5))).strftime("%Y-%m-%d"),
...
def get_stockout_report() -> dict:
    ...
    products = ["Wireless Earbuds", "Running Shoes", "Organic Coffee", "Smart Thermostat",
                "Yoga Mat", "USB-C Cable", "Winter Jacket"]
    stockouts = []
    for _ in range(r.randint(5, 15)):
        ...
        stockouts.append({
            ...
            "eta": ((today + timedelta(days=r.randint(2, 14))).strftime("%Y-%m-%d")
                    if on_order else None),
        })
    ...
    return tool_ok({
        ...
        "items": stockouts,
        "recommendation": ("Prioritize A-class items for expedited reorder. "
