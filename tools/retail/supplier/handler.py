...
SUPPLIER_NAMES = ["GlobalSupply Co", "Pacific Distributors", "Premier Wholesale",
                  "Atlas Trading", "Sunrise Manufacturing", "Metro Imports",
                  "Delta Logistics", "Crown Supply Chain"]
...
def get_supplier_performance(supplier_id: str) -> dict:
    ...
    return tool_ok({
        ...
        "contract_expiry": (today + timedelta(days=r.randint(30, 365))).strftime("%Y-%m-%d"),
...
def list_suppliers(category: str = "all") -> dict:
    ...
    for i, name in enumerate(SUPPLIER_NAMES):
        ...
        suppliers.append({
            ...
            "name": name,
            "category": (r.choice(["Electronics", "Apparel", "Grocery", "Home", "Sports"])
            ...
            "status": r.choice(["PREFERRED", "APPROVED", "APPROVED", "PROBATIONARY"]),
...
def create_purchase_order(supplier_id: str, items: str) -> dict:
    ...
    if not isinstance(item_list, list) or not item_list:
        return tool_error("items must be a non-empty JSON array of "
                          '{"sku", "quantity", "unit_price"} objects')
    total = sum(float(i.get("quantity", 0)) * float(i.get("unit_price", 0)) for i in item_list)
    if total <= 0:
        return tool_error("Order total must be positive; check quantity and unit_price fields")
    ...
    return tool_ok({
        ...
        "estimated_delivery": (today + timedelta(days=r.randint(5, 21))).strftime("%Y-%m-%d"),
...
def get_supplier_risk_report() -> dict:
    ...
    return tool_ok({
        ...
        "top_risks": [
            {"risk": "Single-source for critical electronics components", "severity": "HIGH",
             "mitigation": "Qualify alternate supplier by Q2"},
            {"risk": "3 suppliers in weather-risk zone", "severity": "MEDIUM",
             "mitigation": "Increase safety stock for affected SKUs"},
            {"risk": "Lead time increasing for apparel suppliers", "severity": "MEDIUM",
