...
def get_pricing_analysis(sku: str) -> dict:
    ...
    for name in COMPETITORS:
        ...
        competitors.append({"name": name, "price": comp_price,
    ...
    return tool_ok({
        ...
        "market_position": "BELOW_AVERAGE" if our_price < market_avg else "ABOVE_AVERAGE",
...
def optimize_pricing(sku: str, objective: str) -> dict:
    ...
    if objective not in PRICE_OBJECTIVES:
        return tool_error(f"Invalid objective: {objective}", valid=sorted(PRICE_OBJECTIVES))
    ...
    new_units = max(0, round(daily_units * (1 + (current_price - optimal_price) / current_price * 1.5)))
    ...
    return tool_ok({
        ...
        "impact": {
            "revenue_change_pct": round(
                (optimal_price * new_units / (current_price * daily_units) - 1) * 100, 1),
            "margin_change_pct": round(
                ((optimal_price - cost) * new_units / ((current_price - cost) * daily_units) - 1) * 100, 1),
        },
    }, simulated=True)
...
def get_competitive_intelligence() -> dict:
    ...
    return tool_ok({
        ...
        "opportunities": [
            {"type": "PRICE_INCREASE", "skus": r.randint(20, 80),
             "potential_margin_gain": round(r.uniform(10000, 100000), 2)},
            {"type": "COMPETITIVE_MATCH", "skus": r.randint(10, 40),
             "potential_revenue_gain": round(r.uniform(20000, 150000), 2)},
            {"type": "CLEARANCE_NEEDED", "skus": r.randint(5, 20),
             "inventory_value_at_risk": round(r.uniform(5000, 50000), 2)},
        ],
        "competitor_moves": [
            {"competitor": "Amazon", "action": "Price drops on 45 electronics SKUs",
...
def get_margin_report(category: str = "all") -> dict:
    cats = (["Electronics", "Apparel", "Grocery", "Home & Garden", "Sports"]
            if category.lower() == "all" else [category.title()])
    report = []
    for cat in cats:
    ...
    return tool_ok({
        ...
        "report": report,
        "overall": {
            "blended_gross_margin": round(sum(x["gross_margin_pct"] for x in report) / len(report), 1),
