...
SEVERITY_MAP = {
    "vehicle": {
        ...
        "moderate": (3000, 15000),
        "severe": (15000, 50000),
        "total_loss": (20000, 80000),
    },
    "structural": {
        "minor": (1000, 5000),
        "moderate": (5000, 25000),
        "severe": (25000, 100000),
        "total_loss": (50000, 500000),
    },
    "water": {
        "minor": (500, 2000),
        "moderate": (2000, 15000),
        "severe": (15000, 75000),
        "total_loss": (30000, 200000),
    },
    "fire": {
        "minor": (1000, 5000),
        "moderate": (5000, 30000),
...
def submit_claim(
    policy_number: str, claim_type: str, description: str, incident_date: str
) -> dict:
    ...
    return tool_ok(
        {
            ...
            "required_documents": [
                ...
                (
                    "Photos of damage"
                    if claim_type in ("auto", "home", "property")
                    else "Medical records"
                ),
                (
                    "Police report"
                    if claim_type in ("auto", "liability")
                    else "Receipts/invoices"
                ),
                "Signed claim form",
            ],
    ...
    return tool_ok(
        {
            ...
            "filed_date": (today - timedelta(days=stage_idx * 3 + 5)).strftime(
                "%Y-%m-%d"
            ),
...
def assess_damage(claim_id: str, damage_type: str, photos_submitted: int) -> dict:
    ...
    if damage_type not in SEVERITY_MAP:
        return tool_error(
            f"Invalid damage_type: {damage_type}", valid=sorted(SEVERITY_MAP)
        )
    ...
    return tool_ok(
        {
            ...
            "cost_range": {
                "low": round(estimated_cost * 0.8, 2),
