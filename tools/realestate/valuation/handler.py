    ...
    return tool_ok(
        {
            ...
            "summary": {
                ...
                "median_price_per_sqft": round(
                    sum(c["price_per_sqft"] for c in comparables) / len(comparables), 2
                ),
    ...
    active_listings = [
        {
            "address": f"{r.randint(100, 9999)} {r.choice(STREETS)}",
            "list_price": round(
                base_ppsf * r.uniform(1.0, 1.15) * (sqft + r.randint(-400, 400)), -3
            ),
    ...
    return tool_ok(
        {
            ...
            "market_position": {
                "suggested_list_price": round(
                    value_indicated * r.uniform(1.00, 1.05), -3
                ),
...
def calculate_replacement_cost(
    property_type: str, sqft: int, quality_grade: str, year_built: int
) -> dict:
    ...
    return tool_ok(
        {
            ...
            "total_estimated_value": round(
                depreciated_value + land_value + site_improvements, -3
            ),
