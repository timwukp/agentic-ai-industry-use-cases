...
def calculate_cap_rate(purchase_price: float, annual_noi: float) -> dict:
    ...
    if cap_rate > market_avg_cap + 1:
        assessment = (
            "Above-market cap rate suggests potentially higher risk or undervalued "
            "property. Good value opportunity if risks are manageable."
        )
    elif cap_rate < market_avg_cap - 1:
        assessment = (
            "Below-market cap rate indicates premium property or potential "
            "overpayment. Typical of prime locations with lower risk."
        )
    else:
        assessment = (
            "Cap rate is in line with market averages. Risk-return profile is "
    ...
    return tool_ok(
        {
            ...
            "operating_expense_ratio": (
                round((1 - annual_noi / gross_income) * 100, 1)
                if gross_income > 0
                else 0
            ),
            "per_unit_analysis": {
            ...
            "valuation_at_market_cap": (
                round(annual_noi / (market_avg_cap / 100)) if market_avg_cap > 0 else 0
            ),
        },
        simulated=True,
    )
...
def analyze_rental_income(address: str, property_type: str) -> dict:
    ...
    if property_type not in RENT_RANGES:
        return tool_error(
            f"Invalid property_type: {property_type}", valid=sorted(RENT_RANGES)
        )
...
def calculate_roi(
    ...
    r = _rng(
        "roi", purchase_price, down_payment_pct, interest_rate, rental_income, expenses
    )
    ...
    total_5yr_return = round(
        ...
        + (loan_amount - projections[-1]["loan_balance"]),
        2,
    )
    annualized_roi = (
        round(total_5yr_return / total_cash_invested / 5 * 100, 2)
        if total_cash_invested > 0
        else 0
    )
        ...
        comparisons.append(
            {
                ...
                "price_to_rent_ratio": (
                    round(price / annual_rent, 1) if annual_rent > 0 else 0
                ),
                "risk_score": risk_score,
                "risk_level": (
                    "Low"
                    if risk_score <= 3
                    else "Medium" if risk_score <= 6 else "High"
                ),
            }
        )
    ...
    return tool_ok(
        {
            ...
            "recommendation": {
                ...
                "highest_cap_rate": max(comparisons, key=lambda c: c["cap_rate_pct"])[
                    "address"
                ],
                "highest_cash_flow": max(
                    comparisons, key=lambda c: c["annual_cash_flow"]
                )["address"],
                "lowest_risk": min(comparisons, key=lambda c: c["risk_score"])[
                    "address"
                ],
            },
            "summary": {
                "avg_cap_rate": round(
                    sum(c["cap_rate_pct"] for c in comparisons) / len(comparisons), 2
                ),
                "avg_coc_return": round(
                    sum(c["cash_on_cash_return_pct"] for c in comparisons)
                    / len(comparisons),
                    2,
                ),
                "total_capital_required": round(
                    sum(c["purchase_price"] * 0.25 for c in comparisons)
                ),
            },
        },
        simulated=True,
