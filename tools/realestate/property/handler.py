...
STREETS = ["Oak Dr", "Maple Ave", "Elm St", "Cedar Ln", "Pine Rd", "Birch Ct",
           "Walnut Way", "Spruce Blvd", "Willow Ct", "Magnolia Dr", "Hickory St", "Cherry Ln"]
...
def get_property_details(address: str) -> dict:
    ...
    return tool_ok({
        "address": address,
        "property_type": r.choice(["Single Family", "Condo", "Townhouse", "Multi-Family"]),
        "status": r.choice(["Owner Occupied", "Rental", "Vacant", "Owner Occupied"]),
        "characteristics": {
            ...
            "construction": r.choice(["Wood Frame", "Brick", "Stucco", "Stone", "Concrete Block"]),
            "roof_type": r.choice(["Asphalt Shingle", "Tile", "Metal", "Slate"]),
            "foundation": r.choice(["Slab", "Crawl Space", "Full Basement", "Partial Basement"]),
            "heating": r.choice(["Forced Air", "Radiant", "Heat Pump", "Baseboard"]),
            "cooling": r.choice(["Central AC", "Window Units", "Mini-Split", "None"]),
        },
        "features": r.sample([
            "Updated Kitchen", "Hardwood Floors", "Granite Countertops",
            "Stainless Steel Appliances", "In-Ground Pool", "Fireplace",
            "Deck/Patio", "Fenced Yard", "Solar Panels",
            "Smart Home System", "Home Office", "Walk-in Closets",
        ], k=r.randint(4, 8)),
        "tax_information": {
        ...
        "ownership": {
            ...
            "ownership_type": r.choice(["Fee Simple", "Joint Tenancy", "Trust", "LLC"]),
            "purchase_date": (today - timedelta(days=r.randint(365, 3650))).strftime("%Y-%m-%d"),
...
def check_zoning(address: str) -> dict:
    ...
    max_height = r.choice([35, 40, 45, 55, 75]) if is_residential else r.choice([45, 60, 85, 120])

    if is_residential:
        permitted = ["Single-family dwelling", "Home occupation",
                     "Accessory dwelling unit (ADU)", "Community garden"]
        conditional = ["Day care center", "Bed and breakfast", "Religious institution",
                       "Public utility"]
    else:
        permitted = ["Retail", "Restaurant", "Office", "Personal services",
                     "Bank/Financial", "Medical office"]
        conditional = ["Auto repair", "Drive-through", "Liquor store",
                       "Entertainment venue", "Gas station"]
    ...
    return tool_ok({
        ...
        "prohibited_uses": r.sample(["Heavy industrial", "Landfill", "Junkyard",
                                     "Adult entertainment", "Mining"], k=3),
        "development_standards": {
            "max_height_ft": max_height,
            "max_stories": max_height // r.randint(10, 15),
            "floor_area_ratio": (round(r.uniform(0.3, 0.8), 2) if is_residential
                                 else round(r.uniform(1.0, 4.0), 2)),
            "max_lot_coverage_pct": (round(r.uniform(30, 60)) if is_residential
                                     else round(r.uniform(50, 85))),
            "min_lot_size_sqft": (r.choice([5000, 6000, 7500, 8000, 10000]) if is_residential
            ...
            "parking_required": (f"{r.choice([1, 1.5, 2])} spaces per unit" if is_residential
                                 else f"1 per {r.choice([200, 250, 300])} sqft"),
        },
        "overlay_districts": [r.choice([
            "Historic Preservation District", "Flood Zone AE",
            "Transit Oriented Development (TOD)", "Design Review Overlay",
            "Hillside Protection", "None",
        ])],
...
def get_tax_assessment(address: str) -> dict:
    ...
    return tool_ok({
        ...
        "payment_schedule": {
            "installment_1": {"due_date": f"{today.year}-02-28", "amount": round(net_tax / 2, 2)},
            "installment_2": {"due_date": f"{today.year}-07-31", "amount": round(net_tax / 2, 2)},
        },
    }, simulated=True)
...
def search_properties(criteria: str) -> dict:
    ...
    today = _today()
    r = _rng("search_properties", min_price, max_price, beds_min, baths_min,
             zipcode, property_type, today.strftime("%Y-%m-%d"))
    ...
    for _ in range(page_size):
        ...
        dom = r.randint(1, 90)
        original_price = round(price * r.uniform(1.0, 1.1)) if r.random() > 0.5 else price
        listings.append({
            ...
            "status": r.choice(["Active", "Active", "Active", "Pending", "Coming Soon"]),
            "days_on_market": dom,
            "price_per_sqft": round(price / sqft, 2),
            "features": r.sample(["Garage", "Pool", "Updated Kitchen", "Hardwood",
                                  "Fireplace", "Deck", "New Roof", "Solar"],
                                 k=r.randint(2, 5)),
    ...
    return tool_ok({
        ...
        "summary": {
            ...
            "avg_price_per_sqft": round(sum(x["price_per_sqft"] for x in listings)
                                        / len(listings), 2),
            "avg_days_on_market": round(sum(x["days_on_market"] for x in listings)
                                        / len(listings)),
            "pct_with_price_reduction": round(sum(1 for x in listings if x["price_reduced"])
                                              / len(listings) * 100, 1),
        },
    }, simulated=True)
