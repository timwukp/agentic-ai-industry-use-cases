...
def predict_failure(equipment_id: str) -> dict:
    ...
    rul_days = max(1, int(r.gauss(primary["typical_rul_days"],
                                  primary["typical_rul_days"] * 0.3)))
    failure_probability = round(min(0.99, max(0.05, 1 - rul_days / (primary["typical_rul_days"] * 2))), 2)
    confidence = round(r.uniform(0.72, 0.96), 2)
    secondary = r.choice([m for m in FAILURE_MODES if m["mode"] != primary["mode"]])
    risk_level = ("CRITICAL" if rul_days < 14 else "HIGH" if rul_days < 30
                  else "MEDIUM" if rul_days < 60 else "LOW")
    ...
    return tool_ok({
        ...
        "primary_prediction": {
            ...
            "estimated_failure_date": (today + timedelta(days=rul_days)).strftime("%Y-%m-%d"),
        ...
        "contributing_factors": [
            ...
            {"factor": f"Operating hours: {r.randint(5000, 40000)}h", "impact": "MEDIUM"},
...
def analyze_vibration(equipment_id: str) -> dict:
    ...
    peaks = [
        {"frequency_hz": shaft_freq, "amplitude_mm_s": round(r.uniform(0.2, 5.0), 2),
         "label": "1X (Shaft speed)",
         "diagnosis": "Imbalance" if r.random() > 0.5 else "Normal"},
        {"frequency_hz": round(shaft_freq * 2, 2), "amplitude_mm_s": round(r.uniform(0.1, 3.0), 2),
         "label": "2X (Shaft speed)",
         "diagnosis": "Misalignment" if r.random() > 0.5 else "Normal"},
        {"frequency_hz": bpfo, "amplitude_mm_s": round(r.uniform(0.05, 2.5), 2),
         "label": "BPFO (Outer race)",
         "diagnosis": "Outer race defect" if r.random() > 0.6 else "Normal"},
        {"frequency_hz": bpfi, "amplitude_mm_s": round(r.uniform(0.05, 2.0), 2),
         "label": "BPFI (Inner race)",
         "diagnosis": "Inner race defect" if r.random() > 0.7 else "Normal"},
        {"frequency_hz": bsf, "amplitude_mm_s": round(r.uniform(0.02, 1.5), 2),
         "label": "BSF (Ball spin)",
    ...
    return tool_ok({
        ...
        "measurement_point": r.choice(["Drive End Bearing", "Non-Drive End Bearing",
        ...
        "bearing_defect_frequencies": {
            "BPFO_hz": bpfo, "BPFI_hz": bpfi, "BSF_hz": bsf, "FTF_hz": ftf,
        },
        "frequency_peaks": peaks,
        "defects_detected": defects,
        "diagnosis_summary": ("DEFECTS DETECTED: " + ", ".join(d["diagnosis"] for d in defects)
                              if defects
                              else "No significant defects detected. Machine operating normally."),
        "recommendation": ("Schedule bearing replacement within 2 weeks" if defects
...
def detect_anomalies(equipment_id: str, hours: int = 24) -> dict:
    ...
    today = _today()
    r = _rng("detect_anomalies", equipment_id.upper(), hours, today.strftime("%Y-%m-%d"))
    ...
    for sensor in SENSORS:
        ...
        for _ in range(num):
            ...
            anomalies.append({
                "sensor": sensor,
                "timestamp": (today - timedelta(hours=r.uniform(0, hours))).isoformat(),
                "value": round(r.uniform(50, 200), 2),
                "expected_value": round(r.uniform(40, 150), 2),
                "z_score": z_score,
                "severity": ("CRITICAL" if z_score > 4.5 else "WARNING" if z_score > 3.0
                             else "INFO"),
                "detection_method": r.choice(["z-score", "moving_average_deviation",
                                              "isolation_forest"]),
            })
    ...
    return tool_ok({
        ...
        "overall_assessment": ("ANOMALOUS" if any(a["severity"] == "CRITICAL" for a in anomalies)
                               else "WATCH" if anomalies else "NORMAL"),
        "sensors_analyzed": len(SENSORS),
        "recommendation": ("Investigate critical anomalies immediately. Cross-reference with "
                           "maintenance history." if anomalies
...
def get_reliability_metrics(equipment_id: str) -> dict:
    ...
    return tool_ok({
        ...
        "oee_breakdown": {
            ...
            "oee_class": ("World Class" if oee >= 85 else "Good" if oee >= 70
        ...
        "failure_history": {
            ...
            "top_failure_modes": [
                {"mode": "Bearing failure", "count": r.randint(1, 5),
                 "avg_repair_hours": round(r.uniform(2, 12), 1)},
                {"mode": "Seal leak", "count": r.randint(0, 3),
                 "avg_repair_hours": round(r.uniform(1, 6), 1)},
                {"mode": "Electrical fault", "count": r.randint(0, 3),
