...
SUPPLIERS = ["Industrial Parts Direct", "MRO Supply Corp", "OEM Distributor", "BearingWorld"]
...
def check_spare_parts(part_number: str) -> dict:
    ...
    part_info = PART_CATEGORIES.get(prefix, {"category": "General",
                                             "description": f"Spare part {part_number}"})
    ...
    return tool_ok({
        ...
        "manufacturer": r.choice(["SKF", "FAG/Schaeffler", "NSK", "Timken", "Parker",
                                  "Festo", "Gates", "Mobil"]),
        "unit_cost": unit_cost,
...
def order_spare_parts(part_number: str, quantity: int) -> dict:
    ...
    today = _today()
    r = _rng("order_spare_parts", part_number.upper(), quantity, today.strftime("%Y-%m-%d"))
    ...
    return tool_ok({
        ...
        "delivery": {
            ...
            "estimated_delivery": (today + timedelta(days=lead_time_days)).strftime("%Y-%m-%d"),
            "shipping_method": r.choice(["Standard Ground", "Expedited", "Next-Day Air"]),
            "delivery_location": r.choice(["Main Stores (WH-MAIN)", "Line-Side Stock A (WH-LSA)"]),
        },
        "approval": {
...
def get_parts_forecast(equipment_id: str) -> dict:
    ...
    forecast_specs = [
        ("BRG", "Deep groove ball bearing 6205-2RS", "Bearings",
         "Predictive: vibration trend indicates bearing wear", (7, 45), (15, 120)),
        ("SEL", "Mechanical shaft seal 35mm", "Seals",
         "Scheduled: preventive maintenance due", (14, 60), (25, 180)),
        ("LUB", "Synthetic bearing grease EP2 400g", "Lubricants",
         "Scheduled: lubrication interval approaching", (5, 30), (8, 35)),
        ("FLT", "Hydraulic filter element 10 micron", "Filters",
         "Usage-based: filter life approaching limit", (10, 40), (20, 90)),
        ("BLT", "V-belt B68", "Belts",
         "Preventive: belt replacement interval due", (20, 75), (10, 45)),
    ]
    ...
    for prefix, desc, cat, reason, needed_range, cost_range in forecast_specs:
        ...
        part = {
            ...
            "needed_by": (today + timedelta(days=r.randint(*needed_range))).strftime("%Y-%m-%d"),
    ...
    urgent = sum(1 for p in order_needed
                 if (datetime.strptime(p["needed_by"], "%Y-%m-%d")
                     .replace(tzinfo=timezone.utc) - today).days < 14)
    ...
    return tool_ok({
        ...
        "recommendation": (f"Order {len(order_needed)} parts to avoid maintenance delays. "
...
def get_parts_inventory_report() -> dict:
    today = _today()
    categories = [
        {"name": "Bearings"}, {"name": "Seals & Gaskets"}, {"name": "Filters"},
        {"name": "Belts & Hoses"}, {"name": "Lubricants"},
        {"name": "Electrical Components"}, {"name": "Valves & Fittings"},
    ...
    for cat in categories:
        ...
        category_data.append({
            ...
            "excess_stock_value": round(total_value * excess_count / total_skus
                                        * r.uniform(1.5, 3), 2),
    ...
    return tool_ok({
        ...
        "inventory_summary": {
            "total_part_numbers": sum(c["total_part_numbers"] for c in category_data),
            "total_inventory_value": round(total_value, 2),
            "total_stockout_items": total_stockouts,
            "total_excess_value": round(total_excess_value, 2),
            "overall_service_level_pct": round(
                sum(c["service_level_pct"] for c in category_data) / len(category_data), 1),
