...
TASK_TEMPLATES = [
    {
        "task": "Isolate equipment and apply LOTO",
        "duration_min": 15,
        "skill": "General",
    },
    {
        "task": "Perform visual inspection of components",
        "duration_min": 30,
        "skill": "Mechanical",
    },
    {
        "task": "Remove and inspect bearing assembly",
        "duration_min": 60,
        "skill": "Mechanical",
    },
    {
        "task": "Replace worn bearing with new unit",
        "duration_min": 45,
        "skill": "Mechanical",
    },
    {
        "task": "Check shaft alignment using laser alignment tool",
        "duration_min": 30,
        "skill": "Instrumentation",
    },
    {
        "task": "Refill and test lubrication system",
        "duration_min": 20,
        "skill": "Mechanical",
    },
    {
        "task": "Perform electrical insulation resistance test",
        "duration_min": 25,
        "skill": "Electrical",
    },
    {
        "task": "Run vibration baseline measurement",
        "duration_min": 20,
        "skill": "Instrumentation",
    },
    {
        "task": "Remove LOTO and perform test run",
        "duration_min": 30,
        "skill": "General",
    },
    {
        "task": "Document findings and update maintenance records",
        "duration_min": 15,
        "skill": "General",
    },
]
...
def schedule_maintenance(
    equipment_id: str, maintenance_type: str, priority: str
) -> dict:
    maintenance_type, priority = maintenance_type.lower(), priority.lower()
    if maintenance_type not in VALID_TYPES:
        return tool_error(
            f"Invalid maintenance_type: {maintenance_type}", valid=VALID_TYPES
        )
    ...
    scheduled_date = today + timedelta(
        days=PRIORITY_LEAD_DAYS[priority] + r.randint(0, 3)
    )
    duration_hours = round(
        r.uniform(2, 48) if maintenance_type == "corrective" else r.uniform(1, 24), 1
    )
    ...
    return tool_ok(
        {
            ...
            "approval": {
                ...
                "approver": (
                    "Auto-approved (safety-critical)"
                    if priority in ("critical", "high")
                    else "Maintenance Supervisor"
                ),
            },
            "target_completion": (
                today + timedelta(days=PRIORITY_TARGET_DAYS[priority])
            ).strftime("%Y-%m-%d"),
...
def get_maintenance_history(equipment_id: str, months: int = 12) -> dict:
        ...
        records.append(
            {
                ...
                "date": (today - timedelta(days=r.randint(1, months * 30))).strftime(
                    "%Y-%m-%d"
                ),
                "type": mt,
                ...
                "technician": r.choice(
                    ["M. Rodriguez", "S. Chen", "J. Wilson", "A. Petrova", "D. Kim"]
                ),
                "outcome": r.choice(
                    ["COMPLETED", "COMPLETED", "COMPLETED", "PARTIAL", "DEFERRED"]
                ),
            }
        )
    ...
    corrective_pct = (
        sum(1 for x in records if x["type"] in ("CORRECTIVE", "EMERGENCY"))
        / len(records)
        * 100
    )
    ...
    return tool_ok(
        {
            ...
            "summary": {
                ...
                "by_type": {
                    mt: sum(1 for x in records if x["type"] == mt)
                    for mt in maintenance_types
                },
                "reactive_maintenance_pct": round(corrective_pct, 1),
            },
            "patterns": {
                "most_common_issue": r.choice(
                    ["Bearing failure", "Seal wear", "Electrical faults"]
                ),
...
def get_maintenance_calendar(facility_id: str = "all", days: int = 30) -> dict:
    ...
    today = _today()
    r = _rng(
        "maintenance_calendar", facility_id.upper(), days, today.strftime("%Y-%m-%d")
    )
        ...
        scheduled_items.append(
            {
                ...
                "assigned_to": r.choice(
                    ["M. Rodriguez", "S. Chen", "J. Wilson", "A. Petrova"]
                ),
    ...
    return tool_ok(
        {
            ...
            "conflicts": (
                [
                    {
                        "date": (today + timedelta(days=r.randint(1, days))).strftime(
                            "%Y-%m-%d"
                        ),
