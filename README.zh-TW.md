...
圖表，比沒有圖表更糟。18 個識別器（`web/src/lib/chartSpec.ts`）各自重新驗證它預期的資料形狀，
不符就回傳空值，所以無法識別的工具是**不出面板**，而不是猜一個出來。
...
**回覆語言。** Agent 用提問的語言回答。單靠 system prompt 不夠：一筆用中文寫的記憶紀錄每輪都被
注入，被模型當成語言訊號，導致英文起始問題回中文。現在明確地讓**當前訊息**成為權威
（`web/src/lib/replyLanguage.ts`）——用 CJK 對 Latin 的字集判斷並留有裕度，所以夾滿英文股票代號的
技術中文仍判為中文，英文句子裡混進一個中文字仍判為英文。真正混雜到無法判斷時就完全不下指令，
因為下錯指令比不下指令更糟。
