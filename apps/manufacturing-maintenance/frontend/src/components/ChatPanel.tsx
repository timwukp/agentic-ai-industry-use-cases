...
export default function ChatPanel() {
  ...
  const quickActions = [
    'Show equipment status for EQ-CNC-001',
    'Predict failure for main compressor',
    'Analyze vibration data for pump EQ-PMP-003',
    'Show maintenance calendar for next 30 days',
    'Check spare parts for bearing SKF-6205',
  ]
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-800 bg-gray-900/50">
        <div className="flex items-center gap-3">
          <input
            ...
            placeholder="Ask about equipment health, failure predictions, maintenance schedules, or spare parts..."
            className="flex-1 px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="p-3 bg-amber-600 rounded-xl text-white hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        <div className={`text-xs mt-2 ${isUser ? 'text-amber-200' : 'text-gray-500'}`}>
