...
export default function ChatPanel() {
  ...
  const quickActions = [
    ...
    'Check stock for SKU-ELEC-001',
    'Forecast demand for next 30 days',
    'Run ABC analysis',
    'Show supplier risk report',
  ]
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-800 bg-gray-900/50">
        <div className="flex items-center gap-3">
          <input
            ...
            placeholder="Ask about inventory, demand forecasts, suppliers, or reorder recommendations..."
            className="flex-1 px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="p-3 bg-emerald-600 rounded-xl text-white hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        <div className={`text-xs mt-2 ${isUser ? 'text-emerald-200' : 'text-gray-500'}`}>
